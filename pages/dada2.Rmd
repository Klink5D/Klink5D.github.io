---
title: "Morning Period: Applying the DADA2 library pipeline to generate an analysis-ready dataset"
author: ""
date: ""
...
The **Biome-shiny Biodata.pt Crash Course** will consist on two sections: 

   (1) *upstream data analysis* of **16S rRNA gene amplicon NGS data** with **DADA2** R package (section in the morning)
   
   (2) *downstream data analysis* of **16S rRNA gene amplicon NGS data** with the GUI **Biome-Shiny** (section in the afternoon)


>9:30-12:30

   + Introduction to the **upstream data analysis** of *16S rRNA gene amplicon*
   
   + Hands-on **DADA2** pipeline with a small dataset: V4 region of the 16S rRNA gene amplicon (paired-end Illumina MiSeq data)
   
   + Assess the accuracy of **DADA2** pipeline with a *mock community sample*
   
   + Convert the ASV table with taxonomy into a biom format using the command-line python program [biom](http://biom-format.org)
   
   
>14:00-17:30

   + Introduction to the [**Biome-Shiny**:*: A Shiny R app for microbiome visualization*](https://github.com/BioData-PT/Biome-Shiny) 
   
   +  Hands-on **Biome-Shiny** with the **biom** file (ASV + taxonomy tables) generated previously as well as with some metadata 
   
   + Explore the composition, alpha- and beta-diversity 

<br>

At the end of this course you will be able to analyse the V4 region of 16S rRNA gene amplicon from primer- and adapter-trimmed Illumina paired-end reads until community diversity analyses with **DADA2** pipeline, under the R environment, and with the R-based GUI **Biome-Shiny**. 

### Before getting started 

This tutorial is a replication of the [**original DADA2 tutorial**](https://benjjneb.github.io/dada2/tutorial.html) using the same subset of 16S rRNA gene amplicon data from mouse fecal samples published by the [Schloss lab](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3753973/pdf/zam5112.pdf) [@kozich2013development]. This dataset was chosen because it was also used in the [mothur MiSeq SOP](https://www.mothur.org/wiki/MiSeq_SOP) that runs a OTU-clustering based pipeline, and, thus it allows us to compare our results (although we do not have the time to do it during this short introduction course, but you can do it). 
...
#### Requirements: 
   ...
   + [R version 3.5.0](https://www.r-project.org) and [Rstudio](https://rstudio.com) 
   
   + [dada2 v.1.12.1](https://benjjneb.github.io/dada2/index.html) R package and dependencies
...
><p><font>The *upstream data analysis* of 16S rRNA gene amplicon data assumes that the participants have basic knowledge of **R**.</p><font> 
...
The default **R chunk** code opened in *Rstudio* has color-symbolic text:
   ...
   + <span style="color:blue">blue</span>: special *functions()*, like *library()* to import packages/libraries, and numbers
...
#### Import 16S rRNA gene amplicon fastq files
...
As you can see we have 6 types of data inside the directory/folder *'./MiSeq_SOP'*: 
   ...
   + **fastq**: text plain file with fasta DNA sequences and associated Phred quality scores. Each sequence contains 4 lines: 1<sup>st</sup> fastq header that starts with a **'@'** character; 2<sup>nd</sup> fasta DNA sequence; 3<sup>rd</sup> **'+'** sign; and 4<sup>th</sup> quality scores associated with each nucleotide in the fasta sequence. These files are the ones that we are interested in and that will be used throughout this pipeline. The fastq files, with the exception of the files with the preffix **Mock**, are from mouse fecal samples. The first **F3** means *F*emale number *3* and the **D** followed by one **number** means *D*ay and the *number* of weaning. Since we have paired-end 16S rRNA gene amplicon Illumina MiSeq data (250 bp-long reads spanning the V4 region), the tag **R1** denotes *forward* fastq files and the tag **R2** denotes *reverse* fastq files.  
   ...
   + **mouse.time.design**: text plain file with the experimental design of this study. It cointains two columns: (1) **group** column with the samples names (the same name as you will see below) and (2) **time** categorical/factor column assuming two possible levels **Early** (from day 0 to 9) or **Late** (from day 141 to 150) - depending on the weaning day.
   
  + *mouse.dpw.metadata*, *stability.files* and *stability.batch*: text plain files with metadata and commands needed to run the [mothur MiSeq SOP](https://www.mothur.org/wiki/MiSeq_SOP). We will not use this!


Run the R chunk code below to create a path specifying a directory for each forward fastq file with the pattern - `_R1_001.fastq` - and reverse fastq files - `_R2_001.fastq`. Notice that after we had specified the relative path for all the forward and reverse fastq files we used the function `sort()`. This is really important, because it will order forward and reverse fastq files in the same way if they have exactly the same suffix name (of course with the exception of the *_R1_* and *_R2_* tags). All dowstream steps assume that forward and reverse fastq files are ordered. What this means? It means that the 1<sup>st</sup> element of the vector `fastqFwdPath` corresponds to the same sample in the 1<sup>st</sup> element of the vector `fastqRevPath` and so on. 
...
Below you can use a built-in function to check if `fastqFwdPath` and `fastqRevPath` are or not sorted.
...
><p><font size=2>Tip: try to give the *sampleNames* obejct to the function `length()`</p></font>
...
One of the preliminary tasks in the analysis of 16S rRNA gene amplicon NGS data is the analysis of the quality scores (aka Phred (Q) scores) in both, forward and reverse, fastq files. It is based on the interpretation of these plots that you can make a supported decision latter about the parameters that you will choose to filter and trim the fastq files. Therefore, apply the `dada2` function `plotQualityProfile()` in order to get the quality profiles for forward and reverse files. Give as input/argument the vector list of forward and reverse fastq files: `fastqFwdPath` and `fastqRevPath` 
...
Run the next R chunk code to apply the *filtering and trimming* with the `dada2` function `filterAndTrim()`.
...
filtFastqRevPath <- file.path(fastqPath, "filtered", paste0(sampleNames, "_rev_filt.fastq.gz")) # relative file path of rev filtered reads that will be created below

## assign to each file path the sample name
names(filtFastqFwdPath) <- sampleNames 
names(filtFastqRevPath) <- sampleNames

## filter and trim fwd and rev fastq files writing the new filtered files in compressed - '.gz' - format to the directories spcified above
filterTrimReads <- filterAndTrim(fwd = fastqFwdPath, filt = filtFastqFwdPath, rev = fastqRevPath, filt.rev = filtFastqRevPath, truncLen = c(240,160), maxEE = 2, truncQ = 2, maxN = 0, rm.phix = TRUE, compress = TRUE, verbose = FALSE, multithread = TRUE) # the 'filterAndTrim' function performs the filtering and trimming as specified in the parameters and write the output, ie, the fastq filtered files, to the directories specified above and also prints/gives the number of sequences that were kept after filtering/trimming in relation to the initial number of reads 
...
Run the R chunk code below to perform the dereplication of good-quality/filtered forward and reverse reads.
...
Now, it would be nice if you could plot how many unique/dereplicated sequences we got in relation to the beginning. The next piece of code thus that for you. You can run it. The `#` means comment. You should read it to understand what it is happening and if you have any question, please ask us. 

```{r plot unique - abundance, fig.width=18}

### Plot the number of unique sequences in relation to the beginning 

## retrieve the unique fwd
uniqDerepFwd <- sapply(X = derepFwd, FUN = summary)[1,] # 'derepFwd' is a list that contains several different information, 
#e.g., the sequences dereplicated as well as all the sequences equall to the unique sequences etc
#a convenient way to retrieve the info of unique sequences from the total initial sequences is to use the function summary like this: 'summary(derepFwd$F3D0)' or 'summary(derepFwd$F3D0)[1]' - but this requires pass sample by sample to the obj 'derepFwd' (the dollar sign index the elements of the list 'derepFwd')
#to do this automatically for all the samples we can use 'sapply()' 
#that will retrieve all info for the 20 samples that constitute the list 'derepFwd' 
# as you can see the 'uniqDerepFwd' is a vector of 'class(uniqDerepFwd)' 'character'. We want integers, i.e., a numeric vector in order to plot it
uniqDerepFwd <- as.numeric(x = uniqDerepFwd) # this will "coerce" the 'uniqDerepFwd' character vector into a numeric vector

## retrive the total/initial no. of fwd sequences
total <- sapply(X = derepFwd, FUN = summary)[3,]
total <- as.numeric(x = total)

## retrieve the unique rev
uniqDerepRev <- sapply(X = derepRev, FUN = summary)[1,] 
uniqDerepRev <- as.numeric(uniqDerepRev) # for the rev we don't need to calculate the initial/total sequences because it is the same as fwd

samples <- names(x = sapply(X = derepRev, FUN = summary)[1,]) # retrieve the samples name

## add the names 
names(uniqDerepFwd) <- samples
names(uniqDerepRev) <- samples
names(total) <- samples
...
><p><font size=2>Tip: try to reuse some of the R code that we use before such as the R objects: `uniqDerepFwd`, `totalDerep`, `uniqDerepRev` (this will be useful to calculate the percentage) and `samples` (to assign sample names to the vector values).</font></p>
...
names(Unique_Fwd_Perc) <- samples
Unique_Rev_Perc <- uniqDerepRev / total * 100
names(Unique_Rev_Perc) <- samples
...
><p><font size=2>Tip: try to apply the same functions as before for dereplication: `sapply(list, summary)` where you can substitute `list` by `dadaFwd` or `dadaRev` R objects. This will print a matrix, where the first row represents the no. of **denoised sequences** across each sample column. Therefore, you need to applied the sapply function but index only the first row (as we did above). Even more interesting would be to pass this information to the previous matrix in order to compare the original good-quality sequences, dereplicated and denoised. For that you can use the function `rbind(previous_matrix, new_vector).`</font></p>
...
denoisedFwd <- sapply(X = dadaFwd, FUN = summary)[1,]
denoisedFwd <- as.numeric(x = denoisedFwd)

## get the no. of ASVs for rev
denoisedRev <- sapply(X = dadaRev, FUN = summary)[1,]
denoisedRev <- as.numeric(x = denoisedRev)
...
After denoising, we will merge our forward reads with the respective reverse-complement of reverse reads. This means that forward and the  reverse-complement of reverse reads should overlap along a minimum of 12 bases without any mismatch (`maxMismatch` allowed in the `minOverlap` region is `0` by default). By default, if forward and the reverse-complement of reverse reads does not overlap, reads are discarded as well forward and reverse reads that does have their pair. 
...
><p><font size=2>Tip: `sapply((sapply(mergePE, `[`, 2)), sum)` give you the no. of sequences merged. If you remember the R object `filterTrimReads` above contains in the first column the no. of initial reads, *i.e.*, `filterTrimReads[,1]`, and in the second column the no. of reads that passed the filtering. So, to get the no. of sequences lost since the beginning you just need to: (initial - merged) / initial * 100.

<details><summary>*Answer*</summary><p>

The no. of merged sequences varied between (run the R chunk below): 2552-17431 reads. 

Assuming always that samples are sorted in the same way in all R objects, the solution for the second question is: `(filterTrimReads[,1] - sapply((sapply(mergePE, `[`, 2)), sum)) / filterTrimReads[,1] * 100`. Therefore the average no. of sequences lost was: 16%. 

```{r merged reads - solution}

## minimum
min(sapply((sapply(mergePE, `[`, 2)), sum))

## maximum
max(sapply((sapply(mergePE, `[`, 2)), sum))

## Average of percentage of sequences lost 
mean((filterTrimReads[,1] - sapply((sapply(mergePE, `[`, 2)), sum)) / filterTrimReads[,1] * 100)
...
mergedPE <- sapply((sapply(mergePE, `[`, 2)), sum) # get the number of merged reads
...
The next step in our pipeline is the identification and removal of ASVs that may represent chimeric sequences. For this purpose, we will use the `dada2` function `removeBimeraDenovo()` with the method `method = "consensus"`. So this method is based on the function `isBimera()` that relies on the fact that chimeras arise during PCR with incomplete amplifications and subsequent pairing giving origin to chimeric sequences that result from two incomplete true/parent sequences. Well, the assumptions are: chimeric sequences arise from two pre-existing true/parents sequences, and, based on probability, if this happens, it happens probably with the most abundant sequences that are being amplified. Thus, this method ranks all the ASVs sequences and it compares the less abundant sequences with the most abundant sequences. If one of the less abundant ASVs sequences have high identity with two abundant ASVs sequences, it is flagged as **chimeric**. The `method = "consensus"` does this in a sample basis, *i.e.*, it searches for chimeras in each sample and then it removes the flagged *chimeric* ASVs sequences if the same chimeric sequences appeared *flagged* in a sufficiently high no. of samples. It ouputs an **ASV table** without chimeras. 
...
Let's plot since the beginning the no. of sequences kept in each step:

```{r plot no. seqs pipeline, fig.width=18}
...
*Question*: **What are the `rownames(taxTbl)`?**

<details><summary>*Answer*</summary><p>
The row names of the matrix table `taxTbl` are the ASV DNA sequences. This offers all the convenience of being a *consistent label*, but it is hard to manipulate, *e.g.*, if you need to know which is the taxonomy of the 3<sup>rd</sup> most abundant ASV (you will not write the full DNA sequence to index/query ASV and taxonomy tables).
</p></details>

<br>

Next save the `asvTblNoChim` and `taxTbl` R objects in *rds* format (R format). Latter, if you new these objects you can use the inverse function `readRDS()` to import these R objects to R. 
...
asvTblNoChim2 <- asvTblNoChim2[, c(21,1:19)] # we are removing the mock community

## put taxonomy in a compatible biom format
source("./scripts/biodataPtCrashCourse.R") # import R script with built-in functions
taxTbl2 <- tax2biom(taxTbl2) # this function will aggregate all taxonomic levels separated by '; ' with the prefix "R__" (with R being substituted by the taxonomic rank at which belong, like Phylum, e.g., "P__", Class, "C__"). It will return a data frame with 2 columns: "ID" (gene/ASV) and "taxonomy" (full taxonomy path as it appears in QIIME)
...
### Evaluate accuracy (with the *mock community*)

Now we will assess the accuracy of **DADA2**. For this purpose we will assess if **DADA2** was able to identify all the strains in the *mock community* correctly. 
...
*Question*: **How many mock strains were identified in our dataset? Were we able to identify all the strains in the mock community?**
...
<details><summary>*Answer*</summary><p>
In total, it was identified `r compareRefAsvMock` strains in the mock community/sample. Yes, we identified all the 20 mock strains, as expected. Therefore the accuracy of **DADA2** for this sample was 100%.
</p></details>
...
Then, inside the folder `output`, we will call the program `biom` that will be used to convert our ASV table with taxonomy in text plain format into a **Biological Observation Matrix**, aka **biom**, file format [@mcdonald2012biological]. A **biom** file is a *biological observation matrix* file that can contain an OTU table or other gene feature table as well other metadata fields such as, taxonomy data, known as `observation-metadata`. To convert our *ASV table with taxonomy*, run the following command in the terminal: 
